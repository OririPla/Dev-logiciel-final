...
def generate_5_new_photos (img_choosen,lap):
    ...
    #display plot deimage finale, à enlever ici
    """
    print("Your selected face is : \n ")
    decoded_img_final = decodeur.predict(P)
    plt.figure(figsize=(20, 4))
    ax = plt.subplot(2, 10, 2)
    plt.imshow(tf.squeeze(decoded_img_final[nb]))
    plt.gray()
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
    plt.show()
